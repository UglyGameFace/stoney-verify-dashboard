...
export async function fetchRecentTicketEventsForUser(
  ...
  const [flagRows, vcRows, joinRows, auditLogRows, ticketEventRows] =
    await Promise.all([
      safeSupabaseRows(() =>
        supabase
  ...
  const auditLogEvents = safeArray(auditLogRows)
    ...
    .map((row) => {
      ...
      return {
        ...
        actor_name: row?.staff_name || meta?.staff_name || row?.staff_id || "Staff",
        ticket_id: meta?.ticket_id || meta?.source_ticket_id || row?.ticket_id || null,
        metadata: meta,
        _source: "audit_logs",
      };
    });
  ...
  const merged = [
    ...
    ...buildVcSessionEvents(vcRows).filter(matchesUserOrTicket),
