...
function countMatches(haystack, needles) {
  ...
  for (const needle of needles) {
    const cleanNeedle = normalizeText(needle);
    if (cleanNeedle && cleanHaystack.includes(cleanNeedle)) {
      count += 1;
    }
  }
  return count;
...
function reasonHasCodLegacySignals(searchText) {
  return containsAny(searchText, COD_SIGNAL_TERMS);
}
...
function scoreCategoryMatch(ticket, category) {
  ...
  const codSignalCount = countMatches(searchText, COD_SIGNAL_TERMS);
  ...
  if (categoryDescription && searchText.includes(categoryDescription)) {
    score += 12;
    reasons.push("description-exact");
  }
  ...
  if (codSignalCount > 0 && codCategory) {
    score += 180 + codSignalCount * 18;
    reasons.push("cod-legacy-signal");
  }

  if (codSignalCount >= 2 && codCategory) {
    score += 80;
    reasons.push("strong-cod-signal");
  }

  if (codSignalCount > 0 && !codCategory) {
    score -= 35;
    reasons.push("cod-mismatch-penalty");
  }
