...
function friendlyDiscordError(status, bodyText, path) {
  if (status === 401) return "Discord session expired. Sign in again to continue."
  if (status === 403) return "Discord blocked this request. Check your account permissions and selected server."
...
async function protectedDiscordFetch({ scope, path, token, authorization, init = {} }) {
  ...
  const promise = (async () => {
    ...
    if (!res.ok) {
      ...
      throw new Error(friendlyDiscordError(res.status, bodyText, cleanPath))
    }
