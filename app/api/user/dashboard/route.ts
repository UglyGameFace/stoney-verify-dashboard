...
function json(payload: Row, status = 200) {
...
async function fetchRows(supabase: any, table: string, build: (query: any) => any) {
  try {
...
async function fetchOne(supabase: any, table: string, build: (query: any) => any) {
  try {
    const { data } = await build(supabase.from(table).select("*"));
...
function viewerFromSession(session: Row, guildId: string) {
...
function memberOut(row: Row | null, viewer: Row) {
