...
async function createAuditEvent(supabase: ReturnType<typeof createServerSupabase>, ticketId: string, staffName: string, ticket: TicketRow): Promise<void> {
  try {
    await supabase.from("audit_events").insert({ title: "Ticket unclaimed", description: `${staffName} unclaimed ticket ${ticket?.ticket_number || ticketId}`, event_type: "ticket_unclaimed", related_id: ticketId });
  } catch {}
}
...
async function createActivityFeedEvent(supabase: ReturnType<typeof createServerSupabase>, ticket: TicketRow, actorId: string | null, actorName: string, previousAssigneeId: string | null, previousAssigneeName: string | null): Promise<void> {
  ...
  try {
    await supabase.from("activity_feed_events").insert({
      ...
      metadata: { ticket_id: ticket?.id || null, ticket_number: ticket?.ticket_number || null, previous_assignee_id: previousAssigneeId || null, previous_assignee_name: previousAssigneeName || null, staff_id: actorId || null, staff_name: actorName, source: "dashboard_unclaim_route" },
    });
  } catch {}
}
...
export async function POST(_request: Request, context: { params: { id?: string } }) {
  try {
    ...
    await Promise.allSettled([createAuditEvent(supabase, ticketId, actorName, updatedTicket), createActivityFeedEvent(supabase, updatedTicket, actorId, actorName, currentAssigneeId, currentAssigneeName), updateStaffMetrics(supabase, guildId, actorId, actorName)]);
