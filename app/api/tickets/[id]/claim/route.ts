...
async function createAuditEvent(
  ...
  try {
    await supabase.from("audit_events").insert({
      ...
      description: `${staffName} claimed ticket ${ticket?.ticket_number || ticketId}`,
  ...
  try {
    ...
    if (status === "closed" || status === "deleted") {
      return buildJsonResponse(
        { error: `Cannot claim a ${status} ticket.` },
        409,
        refreshedTokens
      );
    }
    ...
    if (status === "claimed" && currentAssigneeId && currentAssigneeId !== actorId) {
      return buildJsonResponse(
        {
          error: "Ticket is already claimed by another staff member.",
