...
function normalizeAttachments(rawAttachments) {
  ...
  for (const row of safeArray(rawAttachments).slice(0, MAX_ATTACHMENTS)) {
...
function buildDiscordReplyPayload({ message, attachments, actorName, ticket }) {
...
function chunkDiscordMessage(content, limit = DISCORD_MESSAGE_LIMIT) {
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ok: false,
        error:
          json?.message || `Discord API error ${response.status}`,
      };
    }

    return { ok: true, data: json };
  } catch (error) {
    return { ok: false, error: error?.message || "Failed to reach Discord API." };
  }
}

async function mirrorReplyToDiscord({ ticket, actorName, message, attachments }) {
  ...
  for (const chunk of chunks) {
    ...
    if (!result.ok) {
      return {
        ...
        mirroredMessageIds: sent.map((row) => row.id),
...
async function insertDashboardStaffMessages(supabase, ticket, actorId, actorName, message, attachments, discordMessages) {
  ...
  const rows = discordMessages.map((discordMessage, index) => ({
    ...
    avatar_url: null,
    content:
      safeArray(discordMessages).length === 1
        ? message
        : index === 0
          ? message
...
async function createAuditEvent(supabase, ticketId, actorName, preview) {
  try {
...
async function createActivityFeedEvent(supabase, ticket, actorId, actorName, message, attachments, mirroredToDiscord) {
  const guildId = normalizeString(ticket?.guild_id || env?.guildId || "");
  if (!guildId) return;
  ...
  const metadata = {
    ...
    attachment_count: safeArray(attachments).length,
    attachments,
