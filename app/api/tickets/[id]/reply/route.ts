...
async function postDiscordMessage(channelId: string, content: string) {
  ...
  if (!token || !channelId || !content) return { ok: false as const, error: "Missing Discord token, channel, or content." };
  ...
  try {
    ...
    if (!response.ok) return { ok: false as const, error: normalizeString(json?.message) || `Discord API error ${response.status}` };
    return { ok: true as const, data: json as DiscordMessageResponse };
  } catch (error) {
    return { ok: false as const, error: error instanceof Error ? error.message : "Failed to reach Discord API." };
  }
}

async function mirrorReplyToDiscord(args: { ticket: TicketRow; actorName: string | null; message: string; attachments: NormalizedAttachment[] }) {
  const { ticket, actorName, message, attachments } = args;
  const channelId = getTicketChannelId(ticket);
  if (!channelId) return { mirroredToDiscord: false, mirroredMessageIds: [] as string[], mirrorError: "Ticket is not linked to a Discord channel.", discordMessages: [] as DiscordMessageResponse[] };
  ...
  for (const chunk of chunks) {
    ...
    if (!result.ok) return { mirroredToDiscord: sent.length > 0, mirroredMessageIds: sent.map((row) => normalizeString(row.id)).filter(Boolean), mirrorError: result.error || "Failed to mirror reply to Discord.", discordMessages: sent };
    sent.push(result.data as DiscordMessageResponse);
  }

  return { mirroredToDiscord: sent.length > 0, mirroredMessageIds: sent.map((row) => normalizeString(row.id)).filter(Boolean), mirrorError: "", discordMessages: sent };
}
...
async function insertDashboardStaffMessages(
  ...
  const rows = discordMessages.map((discordMessage, index) => ({
    ...
    avatar_url: null,
    content: discordMessages.length === 1 ? message : index === 0 ? message : `Continued reply segment ${index + 1}`,
    attachments,
    embeds: [],
  }));
...
async function createActivityFeedEvent(supabase: ReturnType<typeof createServerSupabase>, ticket: TicketRow, actorId: string | null, actorName: string | null, message: string, attachments: NormalizedAttachment[], mirroredToDiscord: boolean) {
  const guildId = normalizeString(ticket?.guild_id);
  if (!guildId) return;
  const metadata = { ticket_id: ticket?.id || null, ticket_number: ticket?.ticket_number || null, channel_id: getTicketChannelId(ticket) || null, channel_name: ticket?.channel_name || null, ticket_status: ticket?.status || null, attachment_count: attachments.length, attachments, mirrored_to_discord: mirroredToDiscord, staff_id: actorId || null, staff_name: actorName || null, source: "dashboard_ticket_reply" };
  try {
    await supabase.from("activity_feed_events").insert({
      ...
      search_text: ["ticket reply", actorName, actorId, ticket?.id, ticket?.ticket_number, ticket?.username, ticket?.user_id, ticket?.channel_name, ticket?.status, message].filter(Boolean).join(" "),
      metadata,
    });
  } catch {}
}

async function bumpTicketFreshness(supabase: ReturnType<typeof createServerSupabase>, ticket: TicketRow) {
  ...
  try {
    ...
    try { await supabase.from("tickets").update({ updated_at: new Date().toISOString() }).eq("id", ticketId).eq("guild_id", guildId); } catch {}
  }
}
...
export async function POST(request: Request, context: { params: { id?: string } }) {
  ...
  try {
    ...
    if (!guildId) return buildJsonResponse({ error: "Select a server before replying to a ticket.", error_code: "selected_server_required", needsServerSelection: true }, 428, session);
    ...
    const body = safeObject<{ message?: string; attachments?: AttachmentInput[] }>(await request.json().catch(() => ({})));
    const message = normalizeMultiline(body?.message);
    const attachments = normalizeAttachments(body?.attachments);

    if (!message) return buildJsonResponse({ error: "Reply message cannot be empty.", error_code: "invalid_request", selectedGuildId: guildId }, 400, session);
    if (message.length > MAX_MESSAGE_LENGTH) return buildJsonResponse({ error: `Reply is too long. Maximum ${MAX_MESSAGE_LENGTH} characters.`, error_code: "invalid_request", selectedGuildId: guildId }, 400, session);
    ...
    if (!ticketIsReplyable(ticket)) return buildJsonResponse({ error: `Cannot reply to a ${getTicketStatus(ticket) || "closed"} ticket.`, selectedGuildId: guildId }, 409, session);

    const { actorId, actorName } = getActorIdentity(session as SessionLike);
    const { data, error } = await supabase.from("ticket_messages").insert({ ticket_id: ticketId, author_id: actorId || "", author_name: actorName, content: message, message_type: "staff", attachments, source: "dashboard" }).select("*").single();
    if (error || !data) return buildJsonResponse({ error: error?.message || "Failed to save reply.", selectedGuildId: guildId }, 500, session);

    const mirrorResult = await mirrorReplyToDiscord({ ticket, actorName, message, attachments });
    await Promise.allSettled([bumpTicketFreshness(supabase, ticket), createActivityFeedEvent(supabase, ticket, actorId, actorName, message, attachments, mirrorResult.mirroredToDiscord), insertDashboardStaffMessages(supabase, ticket, actorId, actorName, message, attachments, mirrorResult.discordMessages || [])]);

    return buildJsonResponse({ ok: true, selectedGuildId: guildId, message: mapTicketMessage(data as TicketMessageRow), mirroredToDiscord: mirrorResult.mirroredToDiscord, mirroredMessageIds: mirrorResult.mirroredMessageIds || [], mirrorError: mirrorResult.mirrorError || "" }, 200, session);
  } catch (error) {
