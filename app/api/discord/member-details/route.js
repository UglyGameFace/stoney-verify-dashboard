...
function normalizeStoredMemberPayload(storedMember = {}) {
  return {
    ...
    cluster_members: safeArray(storedMember?.cluster_members),
    suspicion_flags: safeArray(storedMember?.suspicion_flags),
    risk_reasons: safeArray(storedMember?.risk_reasons),
  };
}
...
function buildAvatarUrl(user, fallback = "") {
  ...
  if (avatar && userId) {
    ...
    return `https://cdn.discordapp.com/avatars/${userId}/${avatar}.${animated ? "gif" : "png"}?size=128`;
  }
...
function chooseUsername(member, storedMember) {
  return safeString(member?.user?.username) || safeString(storedMember?.username) || "";
}

function chooseRoleState(storedMember, inGuild) {
  if (!inGuild) return safeString(storedMember?.role_state, "left_guild");
  return safeString(storedMember?.role_state, "unknown");
}

function chooseRoleStateReason(storedMember, inGuild) {
  if (!inGuild) {
    return safeString(storedMember?.role_state_reason) || "Member is no longer present in the guild.";
  }
  return safeString(storedMember?.role_state_reason);
}

function buildMergedMemberPayload({ userId, guildId, guildMember, storedMember, fullRoles, inGuild }) {
  ...
  const topRoleName = mergedRoles[0]?.name || safeString(normalizedStored?.top_role) || safeString(normalizedStored?.highest_role_name) || null;
  const topRoleId = mergedRoles[0]?.id || safeString(normalizedStored?.highest_role_id) || null;

  return {
    ...normalizedStored,
    guild_id: guildId,
    user_id: safeString(guildMember?.user?.id) || safeString(normalizedStored?.user_id) || userId,
    username,
    display_name: displayName,
    global_name: safeString(guildMember?.user?.global_name) || displayName || "",
    nickname,
    avatar: safeString(guildMember?.user?.avatar) || safeString(normalizedStored?.avatar_hash),
    avatar_hash: safeString(guildMember?.user?.avatar) || safeString(normalizedStored?.avatar_hash),
    avatar_url: buildAvatarUrl(guildMember?.user, safeString(normalizedStored?.avatar_url)),
    ...
    data_health: safeString(normalizedStored?.data_health, inGuild ? "ok" : "left_guild"),
    ...
    previous_usernames: mergeUniqueStrings([safeString(normalizedStored?.username)], normalizedStored?.previous_usernames),
    previous_display_names: mergeUniqueStrings([safeString(normalizedStored?.display_name)], normalizedStored?.previous_display_names),
    previous_nicknames: mergeUniqueStrings([safeString(normalizedStored?.nickname)], normalizedStored?.previous_nicknames),
...
export async function GET(req) {
  ...
  try {
    ...
    const roleMap = new Map(safeArray(roles).map((role) => [String(role.id), role]));
