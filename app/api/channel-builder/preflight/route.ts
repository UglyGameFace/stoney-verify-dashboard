...
function rows(value: unknown): PreflightRow[] {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!selectedGuildId) {
      return dashboardAuthJson({ ok: false, error: "Select a server before using Channel Builder preflight.", error_code: "selected_server_required", needsServerSelection: true }, 428, session);
    }
    if (!guildId || guildId !== selectedGuildId) {
      return dashboardAuthJson({ ok: false, error: "Channel Builder preflight must run inside the currently selected server context.", error_code: "selected_server_mismatch" }, 403, session);
    }
    if (!botApiConfigured()) {
      return dashboardAuthJson({ ok: false, error: "Bot API URL is not configured for Channel Builder preflight.", error_code: "bot_api_not_configured" }, 503, session);
    }
    ...
    if (!dryRun.ok) {
      return dashboardAuthJson({ ok: false, error: "Fix dry-run danger warnings before bot preflight.", error_code: "dry_run_has_dangers", dryRun }, 409, session);
    }
    ...
    if (!queueRows.length) {
      return dashboardAuthJson({ ok: false, error: "There are no preflightable rows.", error_code: "empty_channel_builder_plan", dryRun }, 400, session);
    }

    const botResponse = await callBotApi("/channel-builder/preflight", {
      guild_id: guildId,
    ...
    return dashboardAuthJson({
      ok: true,
      selectedGuildId,
      queueable: Boolean(botResponse.queueable),
      validation_errors: Array.isArray(botResponse.validation_errors) ? botResponse.validation_errors : [],
      preflight: botResponse.preflight || null,
      item_count: Number(botResponse.item_count || queueRows.length),
      dryRun,
    }, 200, session);
  } catch (error) {
