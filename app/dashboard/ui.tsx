...
export default function DashboardUI(props: { staffUser?: StaffUser }) {
  ...
  async function refreshEverything() {
    ...
    try {
      ...
      const [statsRes, tokensRes, timersRes, auditRes, membersRes] = await Promise.all([
        ...
        apiGet<any>(`/api/tokens?limit=200&status=${status}`),
        apiGet<any>("/api/timers?limit=200"),
        apiGet<any>("/api/audit?limit=200"),
        apiGet<any>(`/api/members?limit=500&guild_id=${encodeURIComponent(currentMe.guildId || "")}`),
      ]);
      ...
      try {
        ...
        // keep dashboard alive even if resolver fails
      }
    } catch (error: any) {
  ...
  async function syncGuildMembers() {
    if (!me?.guildId) {
      setErr("Missing guild id for sync.");
      return;
    }
    ...
    try {
      ...
      setSyncMessage(
        `Synced ${response.syncedMembers ?? 0} members, marked ${response.leftGuildMembers ?? 0} as left guild, and refreshed ${response.tokensUpdated ?? 0} tokens.`
      );
  ...
  useEffect(() => {
    (async () => {
      try {
        const tokenRes = await apiGet<any>(`/api/tokens?limit=200&status=${status}`);
        if (!tokenRes?.error) {
          setTokens((tokenRes?.rows || tokenRes?.data || []) as TokenRow[]);
        }
      } catch {
  ...
  const filteredTokens = useMemo(() => {
    ...
    return tokens.filter((row) => {
      const st = effectiveStatus(row).toLowerCase();
            ...
            return (
              <div className="sb-mcard" key={row.token}>
                <div className="top">
                  <div className="sb-row" style={{ gap: 10 }}>
                    <input
                      ...
                      onChange={(event) => setSelected((prev) => ({ ...prev, [row.token]: event.target.checked }))}
                ...
                <div className="grid">
                  ...
                  <div>
                    <span>Expected</span>
                    <span className="sb-pill">{roleStateLabel(row.expected_role_state)}</span>
                  </div>
                ...
                <div className="actions">
                  <button className="sb-btn sb-btn-green" disabled={loading} onClick={async () => {
                    setLoading(true);
                    setErr("");
                    try {
                  ...
                  <button className="sb-btn sb-btn-red" disabled={loading} onClick={async () => {
                    setLoading(true);
                    setErr("");
                    try {
                  ...
                  <button className="sb-btn" disabled={loading} onClick={async () => {
                    setLoading(true);
                    setErr("");
                    try {
  ...
  const MembersCards = () => (
    <div className="sb-card">
      <div style={{ fontWeight: 900, fontSize: 16 }}>Server Members</div>
      <div className="sb-muted" style={{ marginTop: 4, fontSize: 12 }}>
      ...
      <div className="sb-row" style={{ marginTop: 12, flexWrap: "wrap" }}>
        ...
        <select
          ...
          onChange={(event) => setMemberRoleStateFilter(event.target.value)}
          ...
          filteredMembers.map((member) => (
            <div className="sb-mcard" key={`${member.guild_id}:${member.user_id}`}>
              <div className="top">
                ...
                <span className={roleStateClass(member.role_state)}>{roleStateLabel(member.role_state)}</span>
              </div>
              ...
              <div className="actions">
                ...
                <button className="sb-btn sb-btn-green" onClick={() => {
                  setQ(member.user_id);
          ...
          timers.map((timer) => {
            ...
            return (
              <div className="sb-mcard" key={timer.channel_id}>
                ...
                <div className="actions">
                  <button className="sb-btn sb-btn-red" disabled={loading} onClick={() => cancelTimer(timer.channel_id)}>
                    Cancel
                  </button>
            ...
            return (
              <div className="sb-mcard" key={`${row.id || "audit"}-${index}`}>
                ...
                <div className="grid">
                  <div>
                    <span>Actor</span>
                    {actor ? <UserInline user={actor} /> : row.actor_name || (actorId ? `@${shortId(actorId)}` : "—")}
                  </div>
        ...
        {events.length === 0 ? (
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {events.map((event, index) => (
              <div key={index} className="sb-pre">{JSON.stringify(event, null, 2)}</div>
            ))}
          </div>
        )}
      </div>
  ...
  const SettingsCard = () => (
    <div className="sb-card">
      ...
      <div className="sb-row" style={{ marginTop: 14, flexWrap: "wrap" }}>
        <button className="sb-btn sb-btn-green" onClick={syncGuildMembers} disabled={syncing || !me?.guildId}>
          {syncing ? "Syncing…" : "Sync Guild Members"}
        </button>
        <button className="sb-btn" onClick={refreshEverything} disabled={loading}>
          Refresh Dashboard
        </button>
      </div>
      ...
      {syncMessage ? (
        <div className="sb-row" style={{ marginTop: 12 }}>
          <span className="sb-pill ok">{syncMessage}</span>
        </div>
      ) : null}

      <div className="sb-muted" style={{ marginTop: 14, fontSize: 13 }}>
        This version fixes the broken session display, adds a Members page, makes counts less wrong on legacy rows, and adds a real guild sync route.
      </div>
    </div>
  );
    ...
    <>
      ...
      {drawerOpen ? (
        <>
          ...
          <div className="sb-drawer" role="dialog" aria-modal="true">
            ...
            <div className="body">
              {drawerNav.map((item) => (
                <button key={item.k} className={`sb-navbtn${mod === item.k ? " active" : ""}`} onClick={() => {
                  setMod(item.k);
                  setDrawerOpen(false);
                }}>
                  {item.label}
      ...
      <div className="sb-shell">
        ...
        <main className="sb-main">
          ...
          {mod === "overview" ? (
            <div className="sb-card">
              <div style={{ fontWeight: 950, fontSize: 16 }}>Overview</div>
              <div className="sb-muted" style={{ marginTop: 6 }}>
              ...
              <div style={{ marginTop: 12 }} className="sb-row">
                ...
                <button className="sb-btn" onClick={() => setMod("members")}>Open Members</button>
              </div>
        ...
        <div className="sb-bottomnav" aria-label="Bottom navigation">
          <div className="grid">
            {primaryNav.map((item) => (
              <button key={item.k} className={`item${mod === item.k ? " active" : ""}`} onClick={() => setMod(item.k)}>
        ...
        {stickyVisible ? (
          <div className="sb-stickybar">
            ...
            <div className="row" style={{ marginTop: 8 }}>
              <button className="sb-btn sb-btn-green" onClick={() => bulkDecision("APPROVED")} disabled={loading}>✅ Approve selected</button>
              <button className="sb-btn sb-btn-red" onClick={() => bulkDecision("DENIED")} disabled={loading}>⛔ Deny selected</button>
              <button className="sb-btn" onClick={() => bulkDecision("RESUBMIT")} disabled={loading}>🔁 Resubmit selected</button>
            </div>
          </div>
        ) : null}
