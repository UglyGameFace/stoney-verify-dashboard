            ...
            {timers.length === 0 ? (
              ...
              timers.map((t) => (
                <div className="sb-mcard" key={t.channel_id}>
                  <div className="top">
                    <div style={{ fontWeight: 800 }}>#{shortId(t.channel_id)}</div>
                    <span className="sb-pill warn">⏳ {t.hours}h</span>
                  </div>
                  <div className="grid">
                    <div>
                      <span>Owner</span>@{shortId(t.owner_id)}
                    </div>
                    <div>
                      <span>Started</span>{fmt(t.started_at)}
                    </div>
                    <div>
                      <span>Started by</span>{t.started_by ? `@${shortId(t.started_by)}` : "—"}
                    </div>
                    <div>
                      <span>Guild</span>{shortId(t.guild_id)}
                    </div>
                  </div>
                  <div className="actions">
                    <button className="sb-btn sb-btn-red" disabled={loading} onClick={() => cancelTimer(t.channel_id)}>
            ...
            {audit.length === 0 ? (
              ...
              audit.map((a, idx) => (
                <div className="sb-mcard" key={(a.id || "") + idx}>
                  ...
                  <div className="grid">
                    <div>
                      <span>Actor</span>{a.actor_name || (a.actor_id ? `@${shortId(a.actor_id)}` : "—")}
                    </div>
                    <div>
                      <span>ID</span>{a.actor_id ? shortId(a.actor_id) : "—"}
                    </div>
                  </div>
                  <div className="actions">
                    <button className="sb-btn" onClick={() => setModal({ title: "Audit meta", body: a.meta })}>
    ...
    return (
      <div className="sb-card">
        ...
        <div className="sb-muted" style={{ marginTop: 12, fontSize: 12 }}>
          Wired next: Discord username/avatar resolver (IDs → names), VC Sessions + Transcript viewer.
        </div>
      </div>
    );
  }

  const stickyVisible = selectedTokens.length > 0;

  return (
    <>
      {/* Mobile drawer backdrop */}
      {drawerOpen && <div className="sb-drawer-backdrop sb-only-mobile" onClick={() => setDrawerOpen(false)} />}

      <div className="sb-shell">
        <aside className={"sb-sidebar" + (drawerOpen ? " open" : "")}>
        ...
        <main className="sb-main">
          ...
          {(mod === "verifications" || mod === "tokens") && <Queue />}
        ...
        {stickyVisible && (
          <div className="sb-stickybar sb-only-mobile">
            <div className="sb-row" style={{ justifyContent: "space-between" }}>
              <span className="sb-pill">
                Selected: <b>{selectedTokens.length}</b>
              </span>
